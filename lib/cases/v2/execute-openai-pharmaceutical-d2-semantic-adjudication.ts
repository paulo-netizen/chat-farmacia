...
export type OpenAiPharmaceuticalD2SemanticClientV1 = Readonly<{
  responses: Pick<OpenAI['responses'], 'parse'>;
}>;

export type OpenAiPharmaceuticalD2SemanticExecutionConfigV1 = Readonly<{
  model: typeof OPENAI_PHARMACEUTICAL_D2_CANDIDATE_MODEL;
...
function validateConfig(
  input: OpenAiPharmaceuticalD2SemanticExecutionConfigV1,
  ...
  if (source.model !== OPENAI_PHARMACEUTICAL_D2_CANDIDATE_MODEL) {
    configFailure(
      'config.model',
      `must be exactly ${OPENAI_PHARMACEUTICAL_D2_CANDIDATE_MODEL}`,
    );
  }
  if (
    ...
    source.maxOutputTokens > OPENAI_PHARMACEUTICAL_D2_EXECUTION_LIMITS.maxOutputTokens
  ) {
    configFailure(
      'config.maxOutputTokens',
      `must be an integer from 1 to ${OPENAI_PHARMACEUTICAL_D2_EXECUTION_LIMITS.maxOutputTokens}`,
    );
  }
  if (
    ...
    source.timeoutMs < 1 ||
    source.timeoutMs > OPENAI_PHARMACEUTICAL_D2_EXECUTION_LIMITS.maxTimeoutMs
  ) {
    configFailure(
      'config.timeoutMs',
      `must be an integer from 1 to ${OPENAI_PHARMACEUTICAL_D2_EXECUTION_LIMITS.maxTimeoutMs}`,
    );
  }
  return {
...
function providerFailure(path: string, message: string, cause?: unknown): never {
  throw pharmaceuticalD2SemanticErrorV2(
    'PROVIDER_FAILURE',
    'PROVIDER_RESPONSE',
    path,
...
function responseModel(value: unknown): string {
  if (
    ...
    value.length === 0 ||
    value !== value.trim() ||
    value.length > OPENAI_PHARMACEUTICAL_D2_EXECUTION_LIMITS.maxResponseModelLength
  ) {
...
export async function executeOpenAiPharmaceuticalD2SemanticClaimsV1(
  client: OpenAiPharmaceuticalD2SemanticClientV1,
  request: PharmaceuticalD2SemanticRequestV2,
  configInput: OpenAiPharmaceuticalD2SemanticExecutionConfigV1,
): Promise<PharmaceuticalD2SemanticProviderReceiptV2> {
  const config = validateConfig(configInput);
  let params: ReturnType<typeof buildOpenAiPharmaceuticalD2SemanticParamsV1>;
  try {
    params = buildOpenAiPharmaceuticalD2SemanticParamsV1(request);
  } catch (cause) {
    throw pharmaceuticalD2SemanticErrorV2(
      'INTERNAL_VALIDATION_ERROR',
      'REQUEST_BUILD',
      'semanticRequest',
      'could not build the canonical OpenAI D2 request',
      cause,
    );
  }
  ...
  try {
    ...
    throw pharmaceuticalD2SemanticErrorV2(
      ...
      'OpenAI D2 semantic claim detection request failed',
      cause,
    );
  }
  ...
  if (
    ...
    providerFailure('response', 'provider returned a failed response', response.error ?? undefined);
  }
  if (
  ...
  if (findRefusal(response.output) !== undefined) {
    providerFailure('response.output', 'provider refused D2 semantic claim detection');
  }
  if (
  ...
  if (!parsed.success) {
    ...
    throw pharmaceuticalD2SemanticErrorV2(
      ...
      path,
      'provider output does not match the strict D2 contract',
      parsed.error,
    );
  }
