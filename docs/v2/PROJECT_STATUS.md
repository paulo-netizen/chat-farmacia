...
Pendiente: aceptación live M6-D3B desde SMOKE bajo matrix `/10` (`gpt-5.6-terra`). Matrix `/9` queda históricamente **REJECT**: ref 7 apareció con identidad, excerpt y span correctos, pero su representación canónica válida de refs no estaba preregistrada (`RELATED_CLINICAL_REFS_ALTERNATIVE_GAP`). M6-D3R20 — **CLOSED / COMPLETE** offline: `/10` añade exclusivamente la tercera alternativa completa para ref 7; expectation `/2`, comparación exacta, prompt `/4`, request `/2`, C3 request fingerprint, policy/provider/validator, D1 y governance permanecen intactos. M6-D3B sigue **NOT CLOSED — READY FOR MATRIX-10 LIVE ACCEPTANCE FROM SMOKE**. Los históricos `/1`–`/9` no se reclasifican. M6 46% / proyecto 49.37% sin cambios. Se conservan los pendientes de reglas clínicas versionadas, scoring, persistencia e integración global. Véanse el [modelo de evaluación](04_EVALUATION_MODEL.md) y la [matriz live pre-registrada](18_PHARMACEUTICAL_SEMANTIC_LIVE_ACCEPTANCE.md).
