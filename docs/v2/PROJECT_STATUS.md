...
Pendiente: aceptación live M6-D3B desde SMOKE bajo matrix `/13` (`gpt-5.6-terra`). Matrix `/12` queda históricamente **REJECT** por provenance policy sobre-restringida en C3 ref 8: el output observado `[]` es válido para una conclusión basada en ausencia de autoridad contradictoria; C008/C009 son opcionales, no obligatorias. M6-D3R29 — **CLOSED / COMPLETE** offline. M6-D3B sigue **NOT CLOSED — READY FOR MATRIX-13 LIVE ACCEPTANCE FROM SMOKE** y los históricos `/1`–`/12` no se reclasifican. Si `/13` presenta una nueva variación de provenance sin un defecto contractual claro, pequeño y demostrable, no se abrirá `/14` automáticamente: se recomendará `OPEN / VALIDATION DEBT` y continuar el roadmap independiente. M6 46% / proyecto 49.37% sin cambios. Se conservan los pendientes de reglas clínicas versionadas, scoring, persistencia e integración global. Véanse el [modelo de evaluación](04_EVALUATION_MODEL.md) y la [matriz live pre-registrada](18_PHARMACEUTICAL_SEMANTIC_LIVE_ACCEPTANCE.md).
