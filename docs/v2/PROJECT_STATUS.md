...
Pendiente: aceptación live M6-D3B desde SMOKE bajo matrix `/12` (`gpt-5.6-terra`). Matrix `/11` queda históricamente **REJECT**: identidad semántica y literal correctas, provenance inválida por R005 forbidden. D3R26 cerró la causa como `PROMPT/PROVIDER PROVENANCE SELECTION GAP`; expectation `/3` y comparator `/3` permanecen sound. M6-D3R27 — **CLOSED / COMPLETE** offline: prompt D2 `/5` operacionaliza selection proposition-local y diagnostic `/1` conserva refs observadas, required, optional, missing, forbidden y duplicate en orden canónico, sin raw excerpt, transcript, prompt, payload provider, PII ni secretos. M6-D3B sigue **NOT CLOSED — READY FOR PROMPT-V5 MATRIX-12 LIVE ACCEPTANCE FROM SMOKE**. Los históricos `/1`–`/11` no se reclasifican. M6 46% / proyecto 49.37% sin cambios. Se conservan los pendientes de reglas clínicas versionadas, scoring, persistencia e integración global. Véanse el [modelo de evaluación](04_EVALUATION_MODEL.md) y la [matriz live pre-registrada](18_PHARMACEUTICAL_SEMANTIC_LIVE_ACCEPTANCE.md).
