...
Pendiente: aceptación live M6-D3B desde SMOKE bajo matrix `/11` (`gpt-5.6-terra`). Matrix `/10` queda históricamente **REJECT** por `RELATED_CLINICAL_REFS ACCEPTANCE CONTRACT OVERCONSTRAINED`; D3R23 cerró la auditoría como `A. SUFFICIENT`. M6-D3R24 — **CLOSED / COMPLETE** offline: expectation `/3` separa clasificación semántica exacta, `ONE_OF` de spans literales exactos y provenance required/optional/forbidden, con comparator `/3` fail-closed. Prompt D2 `/4`, request D2 `/2`, fingerprint C3, policy/provider/validator, claimId, D1 y governance permanecen intactos. M6-D3B sigue **NOT CLOSED — READY FOR EXPECTATION-V3 MATRIX-11 LIVE ACCEPTANCE FROM SMOKE**. Los históricos `/1`–`/10` no se reclasifican. M6 46% / proyecto 49.37% sin cambios. Se conservan los pendientes de reglas clínicas versionadas, scoring, persistencia e integración global. Véanse el [modelo de evaluación](04_EVALUATION_MODEL.md) y la [matriz live pre-registrada](18_PHARMACEUTICAL_SEMANTIC_LIVE_ACCEPTANCE.md).
